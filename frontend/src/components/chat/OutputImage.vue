  ...
  >
    ...
    <template v-if="row.output.deleted">
      <div class="w-full h-full bg-surface flex items-center justify-center">
        ...
        <span class="text-[11px] font-semibold text-content-muted">Asset supprimé</span>
      </div>
    </template>
    ...
    <template v-else-if="(row.output.status === 'complete' || row.output.status === 'trashed') && row.output.media_id">
      ...
      <div
        v-if="isTrashed"
        class="absolute inset-0 z-chrome flex items-center justify-center bg-base/72 px-3 text-center backdrop-blur-[1px]"
        role="status"
        aria-label="Asset supprimé"
      >
        <div class="rounded-md border border-amber-400/35 bg-base/80 px-2.5 py-1.5 text-[11px] font-semibold text-amber-200">
          Asset supprimé
        </div>
      </div>
