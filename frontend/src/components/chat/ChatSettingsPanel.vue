<template>
  <div class="contents">
    <button
      ...
      class="fixed inset-0 z-modal bg-overlay-backdrop md:hidden"
      @click="$emit('close')"
    />
    <div
      class="fixed inset-y-0 right-0 z-modal flex h-[100dvh] w-[min(20rem,90vw)] flex-shrink-0 flex-col overflow-hidden border-l border-edge bg-base shadow-2xl transition-[transform,width,opacity] duration-200 md:relative md:inset-auto md:z-auto md:h-auto md:shadow-none"
      :class="isVisible ? 'translate-x-0 opacity-100 md:w-80' : 'translate-x-full opacity-0 pointer-events-none md:w-0 md:translate-x-0'"
      :aria-hidden="!isVisible"
      :inert="!isVisible"
    >
      <div class="flex flex-shrink-0 items-center justify-between border-b border-edge-subtle px-3 py-2 md:hidden">
        <span class="text-xs font-semibold text-content-secondary">Chat settings</span>
        <IconButton aria-label="Close chat settings" title="Close chat settings" @click="$emit('close')">
