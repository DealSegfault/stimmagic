...
import { CubeTransparentIcon, MapIcon, PaintBrushIcon, SparklesIcon } from '@heroicons/vue/24/outline'
import CompositionStudio from '../components/references/CompositionStudio.vue'
import InpaintingStudio from '../components/references/InpaintingStudio.vue'
import ReferenceGenerationStudio from '../components/references/ReferenceGenerationStudio.vue'
...
const mainTab = ref<'inpaint' | 'reference' | 'packs'>('inpaint')
...
function switchMainTab(tab: 'inpaint' | 'reference' | 'packs') {
  mainTab.value = tab
...
async function load(preferredElementId?: number) {
  ...
  try {
    ...
    if (['inpaint', 'reference', 'packs'].includes(queryTab)) {
      mainTab.value = queryTab as 'inpaint' | 'reference' | 'packs'
    }
...
<template>
  <div class="h-full overflow-y-auto bg-base custom-scrollbar">
    <main class="mx-auto max-w-[1600px] px-5 py-6 lg:px-8 space-y-6">
      <header class="flex flex-col gap-4 border-b border-edge-subtle pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div class="mb-2 flex items-center gap-2 text-[11px] font-semibold text-accent"><CubeTransparentIcon class="h-4 w-4" /> Références & Inpainting</div>
          <h1 class="text-2xl font-semibold tracking-tight text-content">Studio Visuel & Références de Continuité</h1>
          <p class="mt-1 max-w-3xl text-sm leading-relaxed text-content-secondary">
            Inpainting multi-zone localisé, génération d'images avec références et vues canoniques via AGY CLI.
          </p>
        </div>
      ...
      <nav class="flex w-fit rounded-lg bg-overlay-faint p-1 gap-1" role="tablist" aria-label="Sections du studio">
        <button
          type="button"
          role="tab"
          class="flex items-center gap-2 rounded-md px-4 py-2 text-xs font-semibold transition-colors"
          :class="mainTab === 'inpaint' ? 'bg-accent/20 text-accent' : 'text-content-muted hover:text-content'"
          :aria-selected="mainTab === 'inpaint'"
          @click="switchMainTab('inpaint')"
        >
          <PaintBrushIcon class="h-4 w-4" /> Inpainting Multi-Zone
        </button>
        <button
          type="button"
          role="tab"
          class="flex items-center gap-2 rounded-md px-4 py-2 text-xs font-semibold transition-colors"
          :class="mainTab === 'reference' ? 'bg-accent/20 text-accent' : 'text-content-muted hover:text-content'"
          :aria-selected="mainTab === 'reference'"
          @click="switchMainTab('reference')"
        >
          <SparklesIcon class="h-4 w-4" /> Génération avec Références
        </button>
        <button
          type="button"
          role="tab"
          class="flex items-center gap-2 rounded-md px-4 py-2 text-xs font-semibold transition-colors"
          :class="mainTab === 'packs' ? 'bg-accent/20 text-accent' : 'text-content-muted hover:text-content'"
          :aria-selected="mainTab === 'packs'"
          @click="switchMainTab('packs')"
        >
          <CubeTransparentIcon class="h-4 w-4" /> Packs & Vues Canoniques
        </button>
      </nav>

      <!-- Tab 1: Inpainting Multi-Zone Studio -->
      <div v-if="mainTab === 'inpaint'">
        <InpaintingStudio :project-id="project.id" :packs="packs" @asset-created="load()" />
      </div>

      <!-- Tab 2: Reference Generation Studio -->
      <div v-else-if="mainTab === 'reference'">
        <ReferenceGenerationStudio :project-id="project.id" :packs="packs" @asset-created="load()" />
      </div>

      <!-- Tab 3: Classic Packs & Canonical Views -->
      <div v-else>
