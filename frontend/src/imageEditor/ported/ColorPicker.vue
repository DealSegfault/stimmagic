...
/**
 * Copied from packages/image-editor/src/components/common/ColorPicker.vue.
 *
 * Copied rather than reused: the snapshot editor is frozen, and this picker is
 * a solved piece of product work — the grid / spectrum / palette tabs, the
 * recent-colour memory, the eyedropper, and the drag behaviour on the spectrum
 * were all debugged there and there is no better version to write.
 *
 * Adapted: the two icons it drew from the old editor's icon registry are
 * inlined, since the registry itself is not worth carrying across.
 */
