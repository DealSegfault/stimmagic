...
SPACES = [
    {"id": "kitchen", "label": "Cuisine ouverte", "x": 55, "y": 55, "width": 285, "height": 230},
    {"id": "living", "label": "Salon", "x": 340, "y": 55, "width": 390, "height": 345},
    {"id": "hallway", "label": "Couloir intérieur", "x": 340, "y": 400, "width": 390, "height": 145},
    {"id": "entry", "label": "Entrée", "x": 625, "y": 260, "width": 105, "height": 140},
    {"id": "exterior", "label": "Couloir extérieur", "x": 760, "y": 245, "width": 115, "height": 170},
]
...
MOVEMENT_TERMS = (
    "avance", "traverse", "entre", "recule", "se retourne", "tourne", "s approche",
    "marche", "va dans", "rejoint", "ouvre les rideaux", "retire son visage",
)
...
def _angle_to(first: tuple[float, float], second: tuple[float, float]) -> float:
    return round(math.degrees(math.atan2(second[1] - first[1], second[0] - first[0])), 1)
...
def _infer_location(text: str, previous: str | None) -> tuple[str, str]:
    ...
    if _has(text, "interieur.", "retour interieur"):
        return "entry", "Entrée / porte palière"
    if _has(text, "cuisine", "bouilloire", "tasse", "tiroir", "chaise", "couteau", "plan de travail"):
...
def _shot_profile(text: str) -> tuple[str, str, str]:
    ...
    if _has(text, "insert", "gp ", "gros plan", "close"):
        lens = "85 mm" if not _has(text, "main", "doigt", "tasse", "telephone", "tiroir", "sol") else "50 mm macro"
        return ("ECU" if _has(text, "gp ", "insert") else "CU"), lens, "Fixe, focus verrouillé"
...
def _camera_emotion(text: str, default: str) -> str:
    ...
    if _has(text, "brutal", "terrif", "tension", "livide", "panique", "plus forte", "crie"):
...
def _camera_for(target: tuple[float, float], plan_type: str, movement: str) -> dict[str, Any]:
    distance = {"ECU": 72.0, "CU": 105.0, "MS": 150.0, "WS": 230.0, "POV": 30.0}.get(plan_type, 150.0)
    x = max(35.0, min(VIEW_WIDTH - 35.0, target[0] - distance))
    y = max(35.0, min(VIEW_HEIGHT - 35.0, target[1] + (35.0 if plan_type != "WS" else 80.0)))
    return {
        "x": round(x, 1),
        "y": round(y, 1),
...
def _visible_actors(spatial_text: str, full_text: str, location_id: str) -> list[tuple[str, str, str]]:
    two_mayas = _has(spatial_text, "deux maya", "face a face", "maya la retient", "maya referme la porte")
    third_maya = _has(spatial_text, "troisieme maya")
    exterior_subject = (
        _has(spatial_text, "maya exterieure")
        and not _has(spatial_text, "hors champ", "identique a celle de maya exterieure")
    )
    exterior_speaker_only = "maya exterieure :" in full_text and "maya" not in spatial_text
    ...
    interior_named = (
        ...
        and not _has(spatial_text, "plan de la porte vide", "appartement apparemment vide", "aucun personnage")
        and (not exterior_subject or two_mayas or third_maya or _has(spatial_text, "notre maya", "maya principale"))
    )
    if two_mayas:
        interior_named = True
        exterior_named = True
...
def _gaze_target(text: str, actor_id: str, position: tuple[float, float]) -> tuple[float, float, str]:
...
def _action_beats(description: str) -> list[str]:
    ...
    for clause in clauses:
        cleaned = re.sub(r"\b(?:MAYA(?: EXT[EÉ]RIEURE)?|VOIX|T[EÉ]L[EÉ]PHONE)\s*:\s*", "", clause, flags=re.I)
        cleaned = re.sub(r"\s+", " ", cleaned).strip(" .:—-")
        if len(cleaned) >= 4 and cleaned.lower() not in {"silence", "intérieur", "extérieur"}:
    ...
    if _has(normalized, "hesite"):
        micro_beats.append("Poids retenu sur l’appui arrière; main suspendue avant l’action")
    if _has(normalized, "se retourne", "tourne la tete", "regard descend"):
        micro_beats.append("Le regard part d’abord; tête puis épaules suivent avec un léger décalage")
    if _has(normalized, "livide", "terrif", "essouffl", "respiration rapide"):
        micro_beats.append("Respiration courte visible dans les épaules; appuis instables mais contenus")
    if _has(normalized, "silence"):
        micro_beats.append("Tenir le regard et la position pendant le silence; aucun geste parasite")
    return beats + micro_beats or ["Tenir la position et l’axe de regard établis par le raccord"]
...
def _persisted_blocking(settings: dict[str, Any], source_hash: str) -> dict[str, Any] | None:
    blocking = settings.get("blocking")
    if not isinstance(blocking, dict) or blocking.get("version") != 1 or not blocking.get("frames"):
        return None
...
def _restore_state_from_view(state: dict[str, Any], view: dict[str, Any]) -> None:
    ...
    if frames:
        for actor in frames[-1].get("actors") or []:
            state.setdefault("last_actor_positions", {})[actor["id"]] = (float(actor["x"]), float(actor["y"]))
...
def build_blocking_view(
    ...
    *,
    ...
    description = str(getattr(shot, "description", "") or getattr(shot, "prompt", "") or "")
    ...
    last_positions: dict[str, tuple[float, float]] = state.setdefault("last_actor_positions", {})
    location_id, location_label = _infer_location(location_text, previous_location)
    explicit_location = _has(
        location_text,
        "cuisine", "bouilloire", "tasse", "tiroir", "chaise", "couteau", "plan de travail",
        "fenetre", "rideau", "salon", "table basse", "porte", "judas", "poignee",
        "couloir", "bureau", "salle de bain", "exterieur.", "cut brutal au noir",
    )
    if not explicit_location:
        actor_position = None
        if (
            _has(text, "maya exterieure")
            and not _has(text, "notre maya", "identique a celle de maya exterieure")
        ):
    ...
    fallback_target = {
        "kitchen": "kitchen", "living": "living", "entry": "inside_door",
        "hallway": "hallway", "exterior": "exterior_door", "black": "black",
    ...
    for actor_id, label, initials in visible_actors:
        ...
        if actor_id == "maya" and _has(text, "troisieme maya"):
            ...
            target_name = min(POSITIONS, key=lambda key: math.dist(target, POSITIONS[key]))
        if actor_id == "maya" and any(item[0] == "maya_ext" for item in visible_actors) and target_name == "inside_door":
            target = (645.0, 330.0)
        ...
        if _has(text, "nouvel angle", "deja", "retour") and not moving:
            start = target
        end = target if moving or actor_id not in last_positions or _has(text, "deja", "face a face") else start
    ...
    for second in range(frame_count):
        ...
        spatial_summary = "; ".join(movement_phrases) if movement_phrases else f"Aucun personnage visible; axe maintenu sur {POSITION_LABELS[target_key]}"
        frames.append({
            "index": second,
    ...
    explicit_bridge = _has(text, "nouvel angle", "deja", "retour", "pov", "cut", "exterieur")
    if jumps:
        verdict = "review"
        message = (
