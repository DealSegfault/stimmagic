...
async def build_messages(
    ...
    for i, item in enumerate(items):
        ...
        include_images = (i >= cutoff_user_idx)
        msg = _item_to_message(item, include_images=include_images)
        if msg is not None:
            messages.append(msg)
...
_ELEMENT_HEADER_RE = re.compile(
    r">\s*-\s*(?P<type>location|character|prop)\s*·\s*\*\*(?P<name>.+?)\*\*\s*·\s*`@(?P<ref>[^`]+)`\s*\([^)]*?media_id:\s*(?P<media_id>\d+)",
    re.IGNORECASE,
)


def _extract_element_refs_from_text(text: str) -> List[Dict[str, Any]]:
    """Extract project elements with media_ids from formatted markdown headers."""
    refs = []
    for m in _ELEMENT_HEADER_RE.finditer(text):
        try:
            m_id = int(m.group("media_id"))
            refs.append({
                "filename": f"{m.group('name')}",
                "name": m.group("name"),
                "reference_id": m.group("ref"),
                "media_id": m_id,
...
def _workspace_file_note(f: Dict[str, Any]) -> str:
    ...
    ref_tag = f" (@{f['reference_id']})" if f.get("reference_id") else ""
    note = f'{f.get("name") or f.get("filename") or "image"}{ref_tag} (media_id={f.get("media_id")}'
    if f.get("asset_id"):
        note += f', asset_id={f["asset_id"]}'
...
def _item_to_message(item: ChatItem, include_images: bool = True) -> Dict[str, Any] | None:
    ...
    if item.item_type == "user_message":
        ...
        workspace_files = (metadata or {}).get("workspace_files") or []
        attachments = (metadata or {}).get("attachments") or []
        element_media_ids = (metadata or {}).get("element_media_ids") or []
        element_refs = _extract_element_refs_from_text(text)
        for em_id in element_media_ids:
            if isinstance(em_id, int) and not any(r.get("media_id") == em_id for r in element_refs):
                element_refs.append({"media_id": em_id})
        ...
        if workspace_files or attachments or element_refs:
            ...
            try:
                ws_dir = get_workspace_dir(item.chat_id)
            except Exception:
                ws_dir = None

            all_refs = list(workspace_files)
            known_media_ids = {f.get("media_id") for f in all_refs if isinstance(f, dict) and f.get("media_id")}
            for att in attachments:
                if isinstance(att, dict) and att.get("media_id") and att["media_id"] not in known_media_ids:
                    all_refs.append(att)
                    known_media_ids.add(att["media_id"])
            for el in element_refs:
                if isinstance(el, dict) and el.get("media_id") and el["media_id"] not in known_media_ids:
                    all_refs.append(el)
                    known_media_ids.add(el["media_id"])
            ...
            for f in all_refs:
                ...
                elif ws_dir and filename and (ws_dir / filename).exists():
                    img_path = ws_dir / filename
                elif media_id:
                    try:
                        from core.profile_context import get_current_profile
                        from app_dirs import get_profile_dir
                        prof_dir = get_profile_dir(get_current_profile())
                        media_dir = prof_dir / "objects" / "media" / str(media_id)
                        if media_dir.is_dir():
                            for child in media_dir.iterdir():
                                if child.is_file() and child.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp", ".bmp"}:
                                    img_path = child
                                    break
                    except Exception:
                        pass

                suffix = img_path.suffix.lower() if img_path else Path(filename).suffix.lower()
                ...
                if is_video:
                    ...
                    # immediately before the matching video token in Ref2VA.
                    video_index = len(video_notes) + 1
                ...
                if is_image and include_images and img_path and len(image_blocks) < MAX_ACTIVE_VIEW_IMAGES:
                    try:
                        from utils.image_ops import open_oriented
                        img = open_oriented(img_path)
                        w, h = img.size
                        max_side = 1024
                        if max(w, h) > max_side:
                        ...
                        log.warning(f"Failed to encode user attachment {img_path}: {e}")
                elif is_image:
                    img_index += 1
