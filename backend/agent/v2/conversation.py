...
def _workspace_file_note(f: Dict[str, Any]) -> str:
    ...
    'x.png (media_id=42, asset_id=17, generated by provider:tool, image-to-video of media 41)'
...
def _item_to_message(item: ChatItem, include_images: bool = True) -> Dict[str, Any] | None:
    ...
    if item.item_type == "user_message":
        ...
        if workspace_files or attachments:
            ...
            for f in all_refs:
                ...
                is_image = bool(img_path and img_path.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp", ".bmp"})
                note_str = _workspace_file_note(f) if f.get("filename") and f.get("media_id") else (filename or str(media_id))
                if is_image:
